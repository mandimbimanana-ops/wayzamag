...
const CitySelector = ({ city, setCity, spots, geoCity, geoQuartier }) => {
  ...
  // Compteur spots par ville (robuste)
  const countForVille = (v) => {
    if (v.value === "Toute l'île") return spots.length;
    const key = v.value.toLowerCase().replace(/antananarivo/i, "tana");
    return spots.filter(s => {
      const sv = (s.ville || "").toLowerCase().replace(/antananarivo/i, "tana");
      return sv === key || sv.includes(key) || key.includes(sv.slice(0, 5));
    }).length;
  };
...
const HomeScreen = ({ city, setCity, geoCity, geoQuartier, onSpot, onMag, onTab, ads, spots = SPOTS_FALLBACK, onInscription }) => {
  ...
  const normalizeVille = (v = "") => v.trim().toLowerCase()
    .replace(/antananarivo/i, "tana")
    .replace(/antananarive/i, "tana")
    .replace(/tananarive/i, "tana")
    .replace(/nosy.?bé/i, "nosy be")
    .replace(/sainte.?marie/i, "sainte marie")
    .replace(/île.?sainte.?marie/i, "sainte marie")
    .replace(/fort.?dauphin/i, "taolagnaro");

  const cityKey = normalizeVille(city);
  const byCity = city === "Toute l'île" ? spots
    : spots.filter(s => normalizeVille(s.ville) === cityKey || normalizeVille(s.region) === cityKey);
