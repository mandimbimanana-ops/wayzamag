...
const SpotsSection = () => {
  ...
  const load = async () => {
    ...
    const { data: gerantsData } = await supabase.from("gerants").select("spot_id, code_acces, actif");
    // Enrichir chaque spot avec le code gérant correspondant
    const gerantMap = {};
    (gerantsData || []).forEach(g => { gerantMap[g.spot_id] = g.code_acces; });
    const enriched = (spotsData || []).map(s => ({ ...s, _code_acces: gerantMap[s.id] || null }));
    setSpots(enriched);
    setLoading(false);
  };
  useEffect(() => { load(); }, []);
  ...
  const valider = async (s) => {
    await supabase.from("spots").update({ actif: true }).eq("id", s.id);
    await supabase.from("gerants").update({ actif: true }).eq("spot_id", s.id);
    load();
  };
  const desactiver = async (s) => {
    await supabase.from("spots").update({ actif: false }).eq("id", s.id);
    await supabase.from("gerants").update({ actif: false }).eq("spot_id", s.id);
    load();
  };
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
            {filtered.map(s => (
              <div key={s.id} style={{ ...S.card, borderLeft: `3px solid ${s.actif ? passColor[s.pass] || "#C8920A" : "rgba(231,76,60,0.5)"}` }}>
                <div style={{ display: "flex", gap: 12, alignItems: "flex-start" }}>
                  {(s.photo_principale || s.img) && <img src={s.photo_principale || s.img} style={{ width: 48, height: 48, borderRadius: 8, objectFit: "cover", flexShrink: 0 }} alt="" />}
                  <div style={{ flex: 1 }}>
                    ...
                    {s._code_acces && (
                      <p style={{ fontSize: 10, color: "#C8920A", fontFamily: "monospace", marginTop: 2 }}>🔑 {s._code_acces}</p>
                    )}
...
// ─── SECTION AGENDA (ADMIN) ───────────────────────────────────
const AgendaAdminSection = () => {
  ...
  const EMPTY = {
    titre: "", type: "evenement", spot_id: "", date: "", heure: "",
    description: "", places_max: "", places_prises: 0,
    prix: "", image_url: "", actif: true,
  };
  const [form, setForm] = useState(EMPTY);
  const [saving, setSaving] = useState(false);
  ...
  const load = async () => {
    ...
    const [{ data: ev }, { data: sp }] = await Promise.all([
      supabase.from("evenements").select("*, spots(nom, ville)").gte("date", dateMin).order("date").order("heure"),
      supabase.from("spots").select("id, nom, ville").order("nom"),
    ]);
    setEvents(ev || []); setSpots(sp || []);
    setLoading(false);
  };
  useEffect(() => { load(); }, []);

  const openNew  = () => { setForm(EMPTY); setEditEv(null); setShowForm(true); };
  const openEdit = (e) => { setForm({ ...e }); setEditEv(e); setShowForm(true); };

  const save = async () => {
    setSaving(true);
    const payload = { ...form, places_max: form.places_max ? parseInt(form.places_max) : null };
  ...
  const deleteEv    = async (id) => { if (!window.confirm("Supprimer cet événement ?")) return; await supabase.from("evenements").delete().eq("id", id); load(); };
  ...
  return (
    <div>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 16 }}>
        <div>
          <p style={{ fontSize: 9, color: "#C8920A", letterSpacing: "0.3em", marginBottom: 2, fontFamily: "Arial" }}>PANNEAU</p>
          <h2 style={{ fontSize: 20, fontWeight: 800, fontStyle: "italic", color: "#F4F7F7" }}>Agenda</h2>
        </div>
      ...
      {showForm && (
        <div style={{ ...S.card, borderRadius: 18, padding: 20, marginBottom: 20, border: "0.5px solid rgba(200,146,10,0.35)" }}>
          <p style={{ fontSize: 10, color: "#C8920A", letterSpacing: "0.3em", marginBottom: 14, fontFamily: "Arial" }}>
            {editEv ? "MODIFIER L'ÉVÉNEMENT" : "NOUVEL ÉVÉNEMENT"}
          </p>
          ...
          <div style={{ display: "flex", gap: 6, marginBottom: 12, flexWrap: "wrap" }}>
            {TYPES.map(t => (
              <button key={t.v} onClick={() => setForm({ ...form, type: t.v })} style={{
                padding: "6px 12px", borderRadius: 20, cursor: "pointer", fontSize: 10, fontFamily: "Arial", fontWeight: 700,
                background: form.type === t.v ? (typeColor[t.v] + "33") : "rgba(255,255,255,0.04)",
          ...
          <label style={S.label}>TITRE</label>
          <input placeholder="Ex: Soirée VIP – Orchidea Club" value={form.titre} onChange={e => setForm({ ...form, titre: e.target.value })} style={S.input} />

          <label style={S.label}>SPOT LIÉ</label>
          <select value={form.spot_id} onChange={e => setForm({ ...form, spot_id: e.target.value })} style={{ ...S.input, appearance: "none" }}>
            <option value="">Aucun spot lié</option>
            {spots.map(s => <option key={s.id} value={s.id}>{s.nom} — {s.ville}</option>)}
          </select>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10 }}>
            <div>
              <label style={S.label}>PLACES MAX (optionnel)</label>
              <input type="number" placeholder="∞" value={form.places_max} onChange={e => setForm({ ...form, places_max: e.target.value })} style={S.input} />
            </div>
            <div>
              <label style={S.label}>PRIX (optionnel)</label>
              <input placeholder="Ex: 15 000 Ar" value={form.prix} onChange={e => setForm({ ...form, prix: e.target.value })} style={S.input} />
            </div>
          </div>

          <label style={S.label}>DESCRIPTION (optionnel)</label>
          <textarea placeholder="Détails de l'événement…" value={form.description} onChange={e => setForm({ ...form, description: e.target.value })} style={{ ...S.input, height: 70, resize: "none" }} />

          <ImageUploader bucket="wayza-ads" label="Image (optionnel)" initialUrl={form.image_url || null} onUploaded={url => setForm({ ...form, image_url: url })} />
          ...
          <div style={{ display: "flex", gap: 10 }}>
            <button onClick={save} disabled={saving} style={{ ...S.btnGold, flex: 1, padding: "12px 0", opacity: saving ? 0.6 : 1 }}>
      ...
      {loading ? <p style={{ color: "rgba(244,247,247,0.4)", fontSize: 13 }}>Chargement...</p>
        : filtered.length === 0 ? <p style={{ color: "rgba(244,247,247,0.3)", fontSize: 13, fontStyle: "italic" }}>Aucun événement.</p>
        : (
          <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
            {filtered.map(ev => (
              <div key={ev.id} style={{ ...S.card, borderLeft: `3px solid ${typeColor[ev.type] || "#C8920A"}` }}>
                <div style={{ display: "flex", gap: 12, alignItems: "flex-start" }}>
                  {ev.image_url && <img src={ev.image_url} style={{ width: 48, height: 48, borderRadius: 8, objectFit: "cover", flexShrink: 0 }} alt="" />}
                  <div style={{ flex: 1 }}>
                    <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", flexWrap: "wrap", gap: 6 }}>
                      <div>
                        <p style={{ fontSize: 13, fontWeight: 700, color: "#F4F7F7" }}>{ev.titre}</p>
                        <p style={{ fontSize: 10, color: typeColor[ev.type] || "#C8920A" }}>{ev.type} · {ev.date} {ev.heure && `à ${ev.heure}`}</p>
                        {ev.spots && <p style={{ fontSize: 10, color: "rgba(244,247,247,0.4)", marginTop: 2 }}>📍 {ev.spots.nom} — {ev.spots.ville}</p>}
                        {ev.places_max && <p style={{ fontSize: 10, color: "rgba(244,247,247,0.35)" }}>👥 {ev.places_prises || 0}/{ev.places_max} places</p>}
                        {ev.prix && <p style={{ fontSize: 10, color: "#C8920A", fontWeight: 700 }}>💰 {ev.prix}</p>}
                      </div>
                      <span style={{ fontSize: 9, fontWeight: 700, color: ev.actif ? "#2ECC8A" : "rgba(244,247,247,0.3)", background: ev.actif ? "rgba(46,204,138,0.15)" : "rgba(255,255,255,0.05)", padding: "3px 8px", borderRadius: 10 }}>
                        {ev.actif ? "PUBLIÉ" : "BROUILLON"}
                      </span>
                    </div>
                    <div style={{ display: "flex", gap: 6, marginTop: 10, flexWrap: "wrap" }}>
                      <button onClick={() => openEdit(ev)} style={{ ...S.btnGhost, fontSize: 10, padding: "5px 12px" }}>✏️ MODIFIER</button>
                      <button onClick={() => toggleActif(ev)} style={{ ...S.toggle(ev.actif), fontSize: 10, padding: "5px 12px" }}>
                        {ev.actif ? "DÉPUBLIER" : "PUBLIER"}
                      </button>
                      <button onClick={() => deleteEv(ev.id)} style={{ ...S.btnDanger, padding: "5px 10px" }}>🗑</button>
                    </div>
                  </div>
                </div>
              </div>
...
const DashboardSection = () => {
  ...
  useEffect(() => {
    const load = async () => {
      ...
      const [
        ...
        supabase.from("spots").select("*", { count: "exact", head: true }).eq("actif", true),
        supabase.from("spots").select("*", { count: "exact", head: true }).eq("pass", "Premium").eq("actif", true),
        supabase.from("reservations").select("*", { count: "exact", head: true }).eq("date_reservation", today),
        supabase.from("ads_regie").select("*", { count: "exact", head: true }).in("statut_demande", ["en_attente", "en_production"]),
        supabase.from("ads_regie").select("*", { count: "exact", head: true }).eq("actif", true).lte("date_debut", today).gte("date_fin", today),
        supabase.from("evenements").select("*", { count: "exact", head: true }).gte("date", today).eq("actif", true),
        supabase.from("spots").select("pass, date_debut, date_fin").eq("pass", "Premium").eq("actif", true),
      ]);
      const revenusEst = (revenuData || []).filter(s => {
        if (!s.date_debut || !s.date_fin) return false;
        const fin = new Date(s.date_fin);
        return fin >= new Date();
      }).length * 20000;
      setStats({ totalSpots, spotsActifs, spotsPremium, resasAujourd, demandesAttente, pubsActives, eventsAVenir, revenusEst });
      setLoading(false);
    };
    load();
  }, []);

  const Kpi = ({ label, value, color = "#C8920A", sub }) => (
    <div style={{ background: "rgba(255,255,255,0.03)", border: `0.5px solid ${color}30`, borderLeft: `3px solid ${color}`, borderRadius: 12, padding: "14px 16px" }}>
      <p style={{ fontSize: 9, color: "rgba(244,247,247,0.4)", letterSpacing: "0.2em", fontFamily: "Arial", marginBottom: 6 }}>{label.toUpperCase()}</p>
      <p style={{ fontSize: 28, fontWeight: 900, color, lineHeight: 1, marginBottom: 2 }}>{value ?? "—"}</p>
      {sub && <p style={{ fontSize: 10, color: "rgba(244,247,247,0.3)" }}>{sub}</p>}
    </div>
  );
  ...
  return (
    <div>
      <div style={{ marginBottom: 20 }}>
        <p style={{ fontSize: 9, color: "#C8920A", letterSpacing: "0.3em", marginBottom: 2, fontFamily: "Arial" }}>VUE D'ENSEMBLE</p>
        <h2 style={{ fontSize: 20, fontWeight: 800, fontStyle: "italic", color: "#F4F7F7" }}>Tableau de bord</h2>
        <p style={{ fontSize: 10, color: "rgba(244,247,247,0.35)", marginTop: 4 }}>{new Date().toLocaleDateString("fr-FR", { weekday: "long", day: "numeric", month: "long", year: "numeric" })}</p>
      </div>

      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10, marginBottom: 16 }}>
        <Kpi label="Spots actifs" value={stats.spotsActifs} sub={`sur ${stats.totalSpots} inscrits`} color="#C8920A" />
        <Kpi label="Premium actifs" value={stats.spotsPremium} sub="spots ✦ Premium" color="#E8B840" />
        <Kpi label="Réservations" value={stats.resasAujourd} sub="aujourd'hui" color="#2ECC8A" />
        <Kpi label="Événements" value={stats.eventsAVenir} sub="à venir" color="#3498DB" />
        <Kpi label="Pubs actives" value={stats.pubsActives} sub="en cours" color="#9B59B6" />
        <Kpi label="Demandes" value={stats.demandesAttente} sub="en attente" color={stats.demandesAttente > 0 ? "#E74C3C" : "rgba(244,247,247,0.3)"} />
      </div>

      <div style={{ background: "rgba(200,146,10,0.08)", border: "0.5px solid rgba(200,146,10,0.3)", borderRadius: 14, padding: "16px 18px", marginBottom: 8 }}>
        <p style={{ fontSize: 9, color: "#C8920A", letterSpacing: "0.2em", fontWeight: 700, fontFamily: "Arial", marginBottom: 6 }}>REVENU PREMIUM ESTIMÉ</p>
        <p style={{ fontSize: 32, fontWeight: 900, color: "#C8920A" }}>{stats.revenusEst.toLocaleString("fr-FR")} Ar</p>
        <p style={{ fontSize: 10, color: "rgba(244,247,247,0.35)", marginTop: 4 }}>Basé sur {stats.spotsPremium} spots Premium actifs × 20 000 Ar / 27 j</p>
      </div>

      {stats.demandesAttente > 0 && (
        <div style={{ background: "rgba(231,76,60,0.08)", border: "0.5px solid rgba(231,76,60,0.35)", borderRadius: 12, padding: "12px 16px" }}>
          <p style={{ fontSize: 12, fontWeight: 700, color: "#E74C3C" }}>🔔 {stats.demandesAttente} demande{stats.demandesAttente > 1 ? "s" : ""} en attente de traitement</p>
          <p style={{ fontSize: 10, color: "rgba(244,247,247,0.4)", marginTop: 4 }}>Allez dans l'onglet Demandes pour les traiter.</p>
        </div>
      )}
    </div>
  );
  ...
  return (
    <div style={{ minHeight: "100vh", background: "#0D2B30", color: "#F4F7F7", fontFamily: "'Georgia', 'Times New Roman', serif" }}>
      ...
      <div style={{ padding: "20px 16px", maxWidth: 680, margin: "0 auto", paddingBottom: 60 }}>
        ...
        {section === "agenda"     && <AgendaAdminSection />}
