          ...
          {CATEGORIES.map(cat => {
            ...
            return (
              <div key={cat.id} onClick={handleCatClick} style={{
                ...
                width: 128,
                background: active ? `${cat.glow}22` : isEmpty ? "rgba(255,255,255,0.015)" : "rgba(255,255,255,0.04)",
                border: `0.5px solid ${active ? cat.glow : cat.glow + (isEmpty ? "20" : "40")}`,
    ...
    <div className="wayza-app-root" style={{
      ...
      <div className="wayza-main-layout" style={{ flex: 1, overflow: "hidden", display: "flex", flexDirection: isDesktop ? "row" : "column", minHeight: 0 }}>
        ...
        <div className="wayza-sidebar" style={{ display: isDesktop ? "flex" : "none", flexDirection: "column", width: 240, minWidth: 240, background: "rgba(0,0,0,0.3)", borderRight: "0.5px solid rgba(200,146,10,0.15)", padding: "24px 16px", gap: 4, overflowY: "auto" }}>
          ...
          {/* Nav items (home, urg) */}
          {navItems.filter(i => i.id !== "mag").map(item => {
      ...
      <div className="wayza-bottom-nav" style={{ position: "absolute", bottom: 0, left: 0, right: 0, padding: isMobileLandscape ? "6px 20px 8px" : "12px 20px 20px", background: "linear-gradient(to top, #0D2B30 60%, transparent)", flexShrink: 0, display: isDesktop ? "none" : "block" }}>
        <div style={{ background: "rgba(0,0,0,0.75)", backdropFilter: "blur(20px)", border: "0.5px solid rgba(255,255,255,0.1)", borderRadius: 50, padding: "10px 16px", display: "flex", justifyContent: "space-around", alignItems: "center" }}>
          {navItems.map(item => {
            const active = tab === item.id && !selectedSpot;
            return (
              <button key={item.id} onClick={() => { setTab(item.id); setSelectedSpot(null); setSelectedArticle(null); }} style={{
                ...
                {item.id === "mag" ? (
                  <div style={{
                    width: 44, height: 44, marginTop: -22,
