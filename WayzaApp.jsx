...
const SPOTS_FALLBACK = [
  ...
  { id:3,  nom:"Villa Pandanus",          ville:"Sainte Marie",  region:"Analanjirofo", cat:"Fast, Grill & Resto",   pass:"Standard", slogan:"La douceur comme art de vivre",   wa:"+261320011003", description:"La Villa Pandanus incarne le raffinement discret des adresses qui n'ont rien à prouver.", gps:"-17.0500, 49.8200", img:"https://images.unsplash.com/photo-1414235077428-338989a2e8c0?w=600&q=80", photos:[], social:{} },
  { id:4,  nom:"Sakamanga Nosy Be",       ville:"Nosy Be",       region:"Diana",        cat:"Fast, Grill & Resto",   pass:"Gold",     slogan:"La table de l'archipel",          wa:"+261320022001", description:"Face à l'Ile Ronde, la maison Sakamanga est un sanctuaire gastronomique.", gps:"-13.3167, 48.2667", img:"https://images.unsplash.com/photo-1504674900247-0877df9cc836?w=600&q=80", photos:[], social:{} },
  { id:5,  nom:"Orchidea Club",           ville:"Nosy Be",       region:"Diana",        cat:"Lounge-K & Night Club", pass:"Gold",     slogan:"La nuit commence ici",            wa:"+261320022002", description:"L'Orchidea Club est l'épicentre de la vie nocturne premium de Nosy Be.", gps:"-13.3200, 48.2700", img:"https://images.unsplash.com/photo-1516450360452-9312f5e86fc7?w=600&q=80", photos:[], social:{} },
  ...
  { id:7,  nom:"Fort Dauphin Sands",      ville:"Taolagnaro",    region:"Anosy",        cat:"Fast, Grill & Resto",   pass:"Gold",     slogan:"Le bout du monde à table",        wa:"+261320033001", description:"Au pied des massifs de l'Anosy, Fort Dauphin Sands propose une expérience culinaire sauvage et sophistiquée.", gps:"-25.0333, 46.9833", img:"https://images.unsplash.com/photo-1533777857889-4be7c70b33f7?w=600&q=80", photos:[], social:{} },
  { id:8,  nom:"Waves & Grace",           ville:"Taolagnaro",    region:"Anosy",        cat:"Fit, Body & Sport",     pass:"Gold",     slogan:"Surfer l'excellence",             wa:"+261320033002", description:"Waves & Grace est l'unique surf lodge premium du Grand Sud malgache.", gps:"-25.0500, 46.9700", img:"https://images.unsplash.com/photo-1502680390469-be75c86b636f?w=600&q=80", photos:[], social:{} },
  ...
  { id:10, nom:"Le Jardin des Délices",   ville:"Antananarivo",  region:"Analamanga",   cat:"Fast, Grill & Resto",   pass:"Gold",     slogan:"La capitale en saveurs",          wa:"+261320044001", description:"Caché derrière les hauts murs d'un riad d'Ambatonakanga, Le Jardin des Délices est la table de référence des Hautes Terres.", gps:"-18.9167, 47.5167", img:"https://images.unsplash.com/photo-1424847651672-bf20a4b0982b?w=600&q=80", photos:[], social:{} },
  { id:11, nom:"Sky Bar Zoma",            ville:"Antananarivo",  region:"Analamanga",   cat:"Lounge-K & Night Club", pass:"Gold",     slogan:"La ville à vos pieds",            wa:"+261320044002", description:"Au vingtième étage surplombant Analakely, le Sky Bar Zoma offre le panorama le plus envoûtant de la capitale.", gps:"-18.9100, 47.5300", img:"https://images.unsplash.com/photo-1572116469696-31de0f17cc34?w=600&q=80", photos:[], social:{} },
...
const HomeScreen = ({ city, setCity, onSpot, onMag, ads }) => {
  ...
  const byCity = city === "Toute l'île" ? SPOTS_FALLBACK
      ...
      <div style={{ padding: "0 16px 8px" }}>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 8, marginBottom: 24 }}>
          {CATEGORIES.map(cat => {
            ...
            return (
            <div key={cat.id} onClick={() => setSelectedCat(active ? null : cat.name)} style={{
              background: active ? `${cat.glow}18` : "rgba(255,255,255,0.03)",
              border: `0.5px solid ${active ? cat.glow : cat.glow + "25"}`,
              ...
              {active && <p style={{ fontSize: 8, color: cat.glow, letterSpacing: "0.15em", marginTop: 4, fontWeight: 700 }}>FILTRÉ ✓</p>}
            </div>
          )})}
        </div>

        {/* Résultat filtre catégorie */}
        {selectedCat && (
          <div style={{ marginBottom: 12, display: "flex", alignItems: "center", justifyContent: "space-between" }}>
            <p style={{ fontSize: 10, color: "#C8920A", letterSpacing: "0.2em" }}>{filteredSpots.length} SPOT{filteredSpots.length > 1 ? "S" : ""} · {selectedCat.toUpperCase()}</p>
            <button onClick={() => setSelectedCat(null)} style={{ background: "rgba(200,146,10,0.15)", border: "0.5px solid rgba(200,146,10,0.4)", borderRadius: 20, padding: "4px 12px", color: "#C8920A", fontSize: 10, cursor: "pointer", fontFamily: "inherit" }}>
              Tout voir
            </button>
          </div>
        )}
  ...
  return (
    <div style={{
      ...
      <style>{`
        ...
        ::-webkit-scrollbar-track { background: #0D2B30; }
        ::-webkit-scrollbar-thumb { background: rgba(200,146,10,0.3); border-radius: 2px; }
        * { box-sizing: border-box; margin: 0; padding: 0; }
        input, button, textarea, select { font-family: 'Georgia', 'Times New Roman', serif; }
        input[type="date"], input[type="time"] { color-scheme: dark; }
        body { background: #071A1D; margin: 0; }

        /* MOBILE PORTRAIT : app plein écran */
        .wayza-shell {
          display: flex; flex-direction: column;
          width: 100vw; height: 100dvh;
          position: relative; overflow: hidden;
        }
        .wayza-sidebar { display: none; }
        .wayza-topnav { display: flex; }
        .wayza-content { flex: 1; overflow: hidden; display: flex; flex-direction: column; }
        .wayza-bottomnav { display: flex; }

        /* PAYSAGE MOBILE + TABLETTE + PC : vrai site web */
        @media (min-width: 768px), (orientation: landscape) and (min-width: 600px) {
          body { overflow-y: auto; }
          .wayza-shell {
            width: 100vw; height: auto; min-height: 100dvh;
            flex-direction: column; overflow: visible;
          }
          .wayza-topnav-ticker { position: sticky; top: 0; z-index: 100; }
          .wayza-main-layout {
          ...
          .wayza-sidebar {
            ...
            padding: 24px 20px;
            border-right: 0.5px solid rgba(200,146,10,0.15);
            position: sticky;
            top: 32px;
          ...
          .wayza-bottomnav { display: none !important; }
        }
      `}</style>

      {/* TICKER — pleine largeur tout le temps */}
      <div className="wayza-topnav-ticker" style={{ background: "#000", padding: "5px 0", borderBottom: "0.5px solid rgba(200,146,10,0.2)", overflow: "hidden", height: 26 }}>
        <div style={{ animation: "ticker 32s linear infinite", whiteSpace: "nowrap", display: "inline-block", width: "200%" }}>
      ...
      <div className="wayza-shell">

        {/* LAYOUT PRINCIPAL */}
        <div className="wayza-main-layout" style={{ display: "flex", flexDirection: "column", flex: 1 }}>

          {/* SIDEBAR — visible seulement sur desktop/paysage */}
          <div className="wayza-sidebar">
            <div style={{ marginBottom: 32 }}>
              ...
              <h1 style={{ fontSize: 28, fontWeight: 800, fontStyle: "italic", lineHeight: 1, color: "#F4F7F7" }}>
                WAYZA <span style={{ color: "#C8920A" }}>2026</span>
              </h1>
              <div style={{ width: 32, height: 1.5, background: "#C8920A", marginTop: 8 }} />
            </div>
            ...
            {navItems.map(item => {
              ...
              return (
                <button key={item.id} onClick={() => { setTab(item.id); setSelectedSpot(null); setSelectedArticle(null); }} style={{
                  ...
                  <Icon name={item.icon} size={18} color={active ? "#C8920A" : "rgba(244,247,247,0.4)"} />
                  <span style={{ fontSize: 11, color: active ? "#C8920A" : "rgba(244,247,247,0.5)", letterSpacing: "0.15em", fontFamily: "Arial, sans-serif", fontWeight: 700 }}>
                    {item.label}
                  </span>
                </button>
              );
            ...
            <div style={{ marginTop: 28 }}>
              <p style={{ fontSize: 9, color: "rgba(244,247,247,0.3)", letterSpacing: "0.25em", fontFamily: "Arial", marginBottom: 10 }}>DESTINATIONS</p>
              {VILLES.map(v => (
                <button key={v} onClick={() => setTab("home")} style={{
                  display: "block", width: "100%", textAlign: "left",
                  background: "none", border: "none", cursor: "pointer",
                  padding: "7px 0", fontSize: 12, color: "rgba(244,247,247,0.45)",
            ...
            <div style={{ marginTop: "auto", paddingTop: 24 }}>
              <p style={{ fontSize: 9, color: "rgba(244,247,247,0.2)", letterSpacing: "0.2em", fontFamily: "Arial" }}>WAYZA © 2026</p>
            </div>
          </div>

          {/* CONTENU PRINCIPAL */}
          <div className="wayza-content">

            {/* Header mobile (portrait seulement) */}
            <div className="wayza-topnav" style={{ padding: "14px 20px 8px", flexShrink: 0, justifyContent: "space-between", alignItems: "flex-end", display: selectedSpot ? "none" : "flex" }}>
              <div>
              ...
              <div style={{ background: "rgba(255,255,255,0.05)", border: "0.5px solid rgba(255,255,255,0.1)", borderRadius: "50%", width: 36, height: 36, display: "flex", alignItems: "center", justifyContent: "center" }}>
                <Icon name="map" size={16} color="#C8920A" />
              </div>
            </div>
        ...
        {/* BOTTOM NAV — mobile portrait seulement */}
        <div className="wayza-bottomnav" style={{ position: "fixed", bottom: 0, left: 0, right: 0, padding: "10px 20px 18px", background: "linear-gradient(to top, #0D2B30 70%, transparent)", zIndex: 50 }}>
          <div style={{ background: "rgba(0,0,0,0.8)", backdropFilter: "blur(20px)", border: "0.5px solid rgba(255,255,255,0.1)", borderRadius: 50, padding: "10px 24px", display: "flex", justifyContent: "space-around", alignItems: "center" }}>
            {navItems.map(item => {
              const active = tab === item.id && !selectedSpot;
              return (
                <button key={item.id} onClick={() => { setTab(item.id); setSelectedSpot(null); setSelectedArticle(null); }} style={{
                  ...
                  {item.id === "mag" ? (
                    <div style={{ width: 48, height: 48, marginTop: -26, background: active ? "#C8920A" : "rgba(200,146,10,0.2)", border: `2px solid ${active ? "#C8920A" : "rgba(200,146,10,0.4)"}`, borderRadius: "50%", display: "flex", alignItems: "center", justifyContent: "center" }}>
                      <Icon name="sparkles" size={19} color={active ? "#0D2B30" : "#C8920A"} />
                    </div>
                  ) : (
                    <Icon name={item.icon} size={19} color={active ? "#C8920A" : "rgba(244,247,247,0.35)"} />
                  )}
                  {item.id !== "mag" && (
