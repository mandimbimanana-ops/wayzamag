...
function useLayout() {
  const getLayout = () => {
    if (typeof window === "undefined") return { isDesktop: false, isLandscape: false };
    const w = window.innerWidth;
    const h = window.innerHeight;
    const landscape = w > h;
    // Vrai desktop/tablette : largeur >= 768 ET pas un petit mobile en paysage
    const isDesktop = w >= 768 && (w >= 1024 || !landscape || h >= 500);
    return { isDesktop, isLandscape: landscape, isMobileLandscape: landscape && w < 1024 };
  };
...
const SpotDetailScreen = ({ spot, onBack }) => {
  ...
  return (
    <div style={{ flex: 1, overflowY: "auto", overflowX: "hidden", paddingBottom: 120, minHeight: 0, height: "100%" }}>
      {/* Galerie photos */}
...
// ─── ÉCRAN ACCUEIL ────────────────────────────────────────────
const HomeScreen = ({ city, setCity, onSpot, onMag, onTab, ads, spots = SPOTS_FALLBACK }) => {
  const [selectedCat, setSelectedCat] = useState(null);

  const byCity = city === "Toute l'île" ? spots
    : city === "Tana" ? spots.filter(s => s.ville === "Antananarivo")
    : spots.filter(s => s.ville === city);
  ...
  return (
    <div style={{ flex: 1, overflowY: "auto", overflowX: "hidden", paddingBottom: 120, minHeight: 0, height: "100%" }}>
      ...
      <div style={{ padding: "0 16px 8px" }}>
        ...
        <div className="wayza-cats-grid" style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 8, marginBottom: 16 }}>
          {CATEGORIES.map(cat => {
            const active = selectedCat === cat.name;
            // Compter les spots disponibles pour cette catégorie dans la ville sélectionnée
            const spotCount = byCity.filter(s => s.cat === cat.name).length;
            const isNavTab = cat.name === "WAYZA MAG" || cat.name === "Urgences";
            const isEmpty = !isNavTab && spotCount === 0;

            const handleCatClick = () => {
              if (cat.name === "WAYZA MAG") { onTab("mag"); return; }
              if (cat.name === "Urgences")  { onTab("urg"); return; }
              if (!isEmpty) setSelectedCat(active ? null : cat.name);
            };

            return (
              <div key={cat.id} onClick={handleCatClick} style={{
                background: active ? `${cat.glow}18` : isEmpty ? "rgba(255,255,255,0.015)" : "rgba(255,255,255,0.03)",
                border: `0.5px solid ${active ? cat.glow : cat.glow + (isEmpty ? "15" : "25")}`,
                borderLeft: `2px solid ${isEmpty ? cat.glow + "40" : cat.glow}`,
                borderRadius: 14, padding: "14px", cursor: isEmpty ? "default" : "pointer",
                transform: active ? "scale(0.97)" : "scale(1)",
                transition: "all 0.15s",
                opacity: isEmpty ? 0.45 : 1,
                position: "relative",
              }}>
                <div style={{ fontSize: 20, marginBottom: 6, color: isEmpty ? cat.glow + "80" : cat.glow }}>{cat.icon}</div>
                <p style={{ fontSize: 11, fontWeight: 700, color: isEmpty ? "rgba(244,247,247,0.4)" : "#F4F7F7", lineHeight: 1.3, marginBottom: 2 }}>{cat.name}</p>
                ...
                {!active && !isNavTab && spotCount > 0 && (
                  <p style={{ fontSize: 8, color: "rgba(244,247,247,0.25)", marginTop: 4 }}>{spotCount} spot{spotCount > 1 ? "s" : ""}</p>
                )}
                {isEmpty && (
                  <p style={{ fontSize: 8, color: "rgba(244,247,247,0.25)", letterSpacing: "0.1em", marginTop: 4 }}>BIENTÔT</p>
                )}
                {isNavTab && (
                  <p style={{ fontSize: 8, color: cat.glow + "aa", letterSpacing: "0.1em", marginTop: 4 }}>→ VOIR</p>
                )}
              </div>
            );
          })}
        ...
        {bannerAds[0] && <div style={{ marginBottom: 16 }}><BannerAd ad={bannerAds[0]} /></div>}
        ...
        <div className="wayza-spots-grid" style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12 }}>
          {filteredSpots.map((s, idx) => (
            <div key={s.id}>
              ...
              {(idx + 1) % 5 === 0 && bannerAds[1] && (
                <div style={{ marginTop: 12, gridColumn: "1 / -1" }}><BannerAd ad={bannerAds[1]} /></div>
              )}
            </div>
          ))}
        </div>
...
const MagScreen = ({ onArticle }) => (
  <div style={{ flex: 1, overflowY: "auto", overflowX: "hidden", padding: "0 16px 120px", minHeight: 0, height: "100%" }}>
    <div style={{ marginBottom: 20 }}>
...
export default function WayzaPrestigeApp() {
  ...
  // Chargement des spots depuis Supabase (fusion avec fallback)
  useEffect(() => {
    const loadSpots = async () => {
      try {
        const { data, error } = await supabase.from("spots").select("*").eq("actif", true);
        if (!error && data?.length) {
          // Fusionner : spots Supabase prioritaires, compléter avec fallback si absent
        ...
        // Garder SPOTS_FALLBACK en cas d'erreur
      }
    };
    loadSpots();
  }, []);
  ...
  const renderMain = () => {
    ...
    if (tab === "home") return <HomeScreen city={city} setCity={setCity} onSpot={setSelectedSpot} onMag={a => { setSelectedArticle(a); setTab("mag"); }} onTab={t => { setTab(t); setSelectedSpot(null); setSelectedArticle(null); }} ads={ads} spots={spots} />;
    if (tab === "mag") {
  ...
  return (
    <div className="wayza-app-root" style={{
      ...
      {/* Ticker dynamique */}
      <div style={{ background: "#000", padding: "5px 0", borderBottom: "0.5px solid rgba(200,146,10,0.2)", overflow: "hidden", flexShrink: 0 }}>
      ...
      <div className="wayza-main-layout" style={{ flex: 1, overflow: "hidden", display: "flex", flexDirection: isDesktop ? "row" : "column", minHeight: 0 }}>
        {/* Sidebar desktop */}
        <div className="wayza-sidebar" style={{ display: isDesktop ? "flex" : "none", flexDirection: "column", width: 220, minWidth: 220, background: "rgba(0,0,0,0.3)", borderRight: "0.5px solid rgba(200,146,10,0.15)", padding: "24px 16px", gap: 8, overflowY: "auto" }}>
        ...
        {/* Contenu principal */}
        <div className="wayza-main-content" style={{ flex: 1, overflow: "hidden", display: "flex", flexDirection: "column", minHeight: 0, height: 0 }}>
          {renderMain()}
        </div>
      </div>

      {/* Bottom nav */}
      <div className="wayza-bottom-nav" style={{ position: "absolute", bottom: 0, left: 0, right: 0, padding: isMobileLandscape ? "6px 20px 8px" : "12px 20px 20px", background: "linear-gradient(to top, #0D2B30 60%, transparent)", flexShrink: 0, display: isDesktop ? "none" : "block" }}>
