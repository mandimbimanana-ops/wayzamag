...
//  v6 — Landscape : catégories sidebar gauche,
//       Offre standard 27 jours,
//       Upload images par le spot (via Console Gérant)
// ============================================================
...
const SpotDetailScreen = ({ spot, onBack }) => {
  ...
  return (
    <div style={{ flex: 1, overflowY: "auto", overflowX: "hidden", paddingBottom: 120, minHeight: 0 }}>
      {/* Galerie photos */}
      ...
      <div style={{ padding: "0 20px", marginTop: -30, position: "relative" }}>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10, marginBottom: 24 }}>
          {[["Catégorie", spot.cat], ["Abonnement", spot.pass], ["Région", spot.region], ["GPS", spot.gps]].map(([l, v]) => (
        ...
        {isPremium && (socialLinks.whatsapp || socialLinks.facebook || socialLinks.instagram || socialLinks.messenger) && (
        ...
        {/* Réservation Premium uniquement */}
        {isPremium && (
          <>
            {!showResaForm ? (
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10, marginBottom: 24 }}>
...
// ─── PANEL CATÉGORIES (réutilisable portrait + landscape) ─────
const CategoriesPanel = ({ selectedCat, setSelectedCat, byCity, onTab, landscape = false }) => (
  <div style={landscape ? {
    // Sidebar gauche en landscape
    width: 180, minWidth: 180, overflowY: "auto", overflowX: "hidden",
    borderRight: "0.5px solid rgba(200,146,10,0.15)",
    background: "rgba(0,0,0,0.25)",
    padding: "16px 10px",
    display: "flex", flexDirection: "column", gap: 4,
  } : {
    // Grille 2 colonnes en portrait
    marginBottom: 16,
  }}>
    {!landscape && (
    ...
    {landscape && (
      <p style={{ fontSize: 8, color: "#C8920A", letterSpacing: "0.25em", fontWeight: 700, marginBottom: 10, paddingLeft: 4 }}>CATÉGORIES</p>
    )}

    <div style={landscape ? { display: "flex", flexDirection: "column", gap: 4 } : {
        ...
        if (landscape) {
          ...
          return (
            <button key={cat.id} onClick={handleCatClick} style={{
              background: active ? `${cat.glow}18` : "none",
              border: active ? `0.5px solid ${cat.glow}` : "0.5px solid transparent",
              borderLeft: `2px solid ${isEmpty ? cat.glow + "30" : active ? cat.glow : cat.glow + "60"}`,
              borderRadius: 10, padding: "9px 10px", cursor: isEmpty ? "default" : "pointer",
              ...
              <div style={{ flex: 1, minWidth: 0 }}>
                <p style={{ fontSize: 10, fontWeight: 700, color: active ? cat.glow : isEmpty ? "rgba(244,247,247,0.3)" : "#F4F7F7", lineHeight: 1.2, whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis" }}>{cat.name}</p>
                {!isEmpty && !isNavTab && (
                  <p style={{ fontSize: 8, color: "rgba(244,247,247,0.25)", marginTop: 1 }}>{spotCount} spot{spotCount > 1 ? "s" : ""}</p>
                )}
                {isEmpty && <p style={{ fontSize: 8, color: "rgba(244,247,247,0.2)" }}>BIENTÔT</p>}
                {isNavTab && <p style={{ fontSize: 8, color: cat.glow + "99" }}>→ VOIR</p>}
              </div>
              {active && <span style={{ fontSize: 8, color: cat.glow }}>✓</span>}
            </button>
          );
        }

        // Style grille portrait
        return (
...
// ─── ÉCRAN ACCUEIL ────────────────────────────────────────────
const HomeScreen = ({ city, setCity, onSpot, onMag, onTab, ads, spots = SPOTS_FALLBACK, isLandscape }) => {
  ...
  // Contenu spots + bannières (partagé portrait/landscape)
  const SpotsContent = () => (
    <div style={{ padding: isLandscape ? "0 14px 16px" : "0 16px 8px" }}>
      {selectedCat && (
      ...
      <div className="wayza-spots-grid" style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12 }}>
        ...
        {filteredSpots.map(s => (
          <div key={s.id}><SpotCard spot={s} onClick={onSpot} /></div>
        ))}
      </div>
  ...
  // ── MODE LANDSCAPE : catégories sidebar gauche ──
  if (isLandscape) {
    return (
      <div style={{ flex: 1, display: "flex", flexDirection: "row", overflow: "hidden", minHeight: 0 }}>
        ...
        {/* Contenu principal */}
        <div style={{ flex: 1, overflowY: "auto", overflowX: "hidden", paddingBottom: 80, minHeight: 0 }}>
          {/* Hero mag compact landscape */}
          <div style={{ margin: "12px 14px", position: "relative", height: 160, borderRadius: 16, overflow: "hidden", cursor: "pointer" }} onClick={() => onMag(MAG[0])}>
            <img src={MAG[0].img} alt="hero" style={{ width: "100%", height: "100%", objectFit: "cover" }} />
            <div style={{ position: "absolute", inset: 0, background: "linear-gradient(to top, #0D2B30 0%, rgba(13,43,48,0.1) 60%, transparent 100%)" }} />
            <div style={{ position: "absolute", inset: 0, padding: "14px 18px", display: "flex", flexDirection: "column", justifyContent: "flex-end" }}>
              <span style={{ fontSize: 8, fontWeight: 800, letterSpacing: "0.4em", color: "#C8920A", marginBottom: 4 }}>À LA UNE DU MAG</span>
              <h2 style={{ fontSize: 16, fontWeight: 800, color: "#F4F7F7", lineHeight: 1.25, marginBottom: 6, fontStyle: "italic" }}>{MAG[0].titre} — {MAG[0].soustitre}</h2>
            </div>
          </div>
...
// ─── ÉCRAN MAG ────────────────────────────────────────────────
const MagScreen = ({ onArticle, ads }) => {
  // Articles Wayz'Mag depuis Supabase (type "wayzmag")
...
export default function WayzaPrestigeApp() {
    ...
    const loadAds = async () => {
      try {
        ...
        if (data?.length) {
          ...
          if (popup) {
            setTimeout(() => { setActiveAd(popup); setShowPopup(true); }, 2800);
          }
        } else {
  ...
  const tickerText = ads.find(a => a.type === "ticker")?.texte
    || "✦ NOSY BE : Orchidea Club — VIP Premium ✦ SAINTE MARIE : Baie des Baleines Lodge ✦ TAOLAGNARO : Waves & Grace ✦ TANA : Sky Bar Zoma — La ville à vos pieds ✦";
  ...
  const renderMain = () => {
    ...
    if (tab === "home") return <HomeScreen city={city} setCity={setCity} onSpot={setSelectedSpot} onMag={a => { setSelectedArticle(a); setTab("mag"); }} onTab={t => { setTab(t); setSelectedSpot(null); setSelectedArticle(null); }} ads={ads} spots={spots} isLandscape={isMobileLandscape || isDesktop} />;
    if (tab === "mag") {
      if (selectedArticle) return <ArticleScreen article={selectedArticle} onBack={() => setSelectedArticle(null)} />;
      return <MagScreen onArticle={a => setSelectedArticle(a)} ads={ads} />;
    }
