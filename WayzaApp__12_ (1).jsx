...
const CATEGORIES = [
  ...
  { id: 3, name: "Pass & Glow",           icon: "✦",  glow: "#D4A0B0", sub: "Accès privilégié" },
  { id: 4, name: "Wayz'Mag",              icon: "◈",  glow: "#C8920A", sub: "Édition prestige" },
  { id: 5, name: "Fit, Body & Sport",     icon: "◉",  glow: "#1B6B4A", sub: "Luxe actif" },
  { id: 6, name: "Window Shopper",        icon: "◇",  glow: "#6B1A2A", sub: "Créateurs rares" },
      ...
      <div style={{ padding: "0 20px", marginTop: -30, position: "relative" }}>
        ...
        {(spot.adresse || spot.gps) && (
          <div style={{ background: "rgba(255,255,255,0.03)", borderRadius: 14, padding: "12px 14px", marginBottom: 20, border: "0.5px solid rgba(255,255,255,0.08)" }}>
            {spot.adresse && <p style={{ fontSize: 12, color: "rgba(244,247,247,0.6)", marginBottom: spot.gps ? 4 : 0 }}>📍 {spot.adresse}</p>}
            {spot.gps && <p style={{ fontSize: 10, color: "rgba(244,247,247,0.35)", fontFamily: "monospace" }}>GPS : {spot.gps}</p>}
          </div>
        )}
...
// ─── ÉCRAN ACCUEIL ────────────────────────────────────────────
const HomeScreen = ({ city, setCity, onSpot, onMag, onTab, ads, spots = SPOTS_FALLBACK, onInscription }) => {
  ...
  return (
    <div style={{ flex: 1, overflowY: "auto", overflowX: "hidden", paddingBottom: 120, minHeight: 0 }}>
      ...
      <div style={{ padding: "0 16px 8px" }}>
        ...
        <div style={{ display: "flex", gap: 10, overflowX: "auto", scrollSnapType: "x mandatory", scrollbarWidth: "none", WebkitOverflowScrolling: "touch", paddingBottom: 4, marginBottom: 16, marginLeft: -16, paddingLeft: 16, marginRight: -16, paddingRight: 16 }}>
          {CATEGORIES.map(cat => {
            const active = selectedCat === cat.name;
            const spotCount = byCity.filter(s => s.cat === cat.name).length;
            const isNavTab = cat.name === "Wayz'Mag" || cat.name === "Urgences";
            const isEmpty = !isNavTab && spotCount === 0;

            const handleCatClick = () => {
              if (cat.name === "Wayz'Mag") { onTab("mag"); return; }
            ...
            return (
              <div key={cat.id} onClick={handleCatClick} style={{
                ...
                width: 128,
                background: active
                  ? `linear-gradient(135deg, ${cat.glow}28, ${cat.glow}12)`
                  : isEmpty ? "rgba(255,255,255,0.015)" : `linear-gradient(135deg, ${cat.glow}0f, rgba(255,255,255,0.03))`,
                border: `0.5px solid ${active ? cat.glow : cat.glow + (isEmpty ? "20" : "35")}`,
                borderLeft: `3px solid ${isEmpty ? cat.glow + "40" : cat.glow}`,
                borderRadius: 14, padding: "14px 12px", cursor: isEmpty ? "default" : "pointer",
                transform: active ? "scale(0.97)" : "scale(1)",
                transition: "all 0.15s",
                opacity: isEmpty ? 0.45 : 1,
                boxShadow: active
                  ? `0 0 20px ${cat.glow}45, 0 0 8px ${cat.glow}25, inset 0 0 16px ${cat.glow}12`
                  : `0 2px 8px ${cat.glow}15`,
              }}>
                <div style={{ fontSize: 22, marginBottom: 7, color: isEmpty ? cat.glow + "80" : cat.glow }}>{cat.icon}</div>
                <p style={{ fontSize: 11, fontWeight: 700, color: isEmpty ? "rgba(244,247,247,0.4)" : "#F4F7F7", lineHeight: 1.3, marginBottom: 2 }}>{cat.name}</p>
                <p style={{ fontSize: 9, color: "rgba(244,247,247,0.3)", letterSpacing: "0.08em" }}>{cat.sub}</p>
        ...
        {/* Bannière Bas — carrousel en pied de liste */}
    ...
    <div style={{ flex:1, display:"flex", flexDirection:"column", overflow:"hidden", minHeight:0 }}>
      ...
      <div style={{ padding:"16px 20px 12px", flexShrink:0, background:"rgba(0,0,0,0.2)", borderBottom:"0.5px solid rgba(200,146,10,0.15)" }}>
        <div style={{ display:"flex", alignItems:"center", gap:12 }}>
          ...
          <div style={{ flex:1 }}>
            <p style={{ fontSize:8, color:"#C8920A", letterSpacing:"0.35em", fontWeight:700 }}>INSCRIPTION GRATUITE</p>
      ...
      <div style={{ flex:1, overflowY:"auto", padding:"20px 20px 120px", minHeight:0 }}>
        ...
        {etape === 1 && (
          <div>
            ...
            <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:10, marginBottom:20 }}>
              {[
                { v:"Presence", l:"Présence", prix:"Gratuit", desc:"Fiche complète visible · Pub à la carte" },
                ...
                <div key={o.v} onClick={() => set("offre", o.v)} style={{
                  ...
                  <p style={{ fontSize:11, color:"#C8920A", fontWeight:700, marginBottom:5 }}>{o.prix} / 27j</p>
        ...
        {etape === 4 && (
          <div>
            ...
            <label style={SI.label}>COORDONNÉES GPS</label>
            <div style={{ display:"flex", gap:8, marginBottom:10 }}>
              <input placeholder="Ex : -13.3167, 48.2667" value={form.gps} onChange={e=>set("gps",e.target.value)} style={{ ...SI.input, marginBottom:0, flex:1 }} />
            ...
            <p style={{ fontSize:10, color:"rgba(244,247,247,0.3)", marginBottom:18 }}>📍 pour détecter automatiquement votre position.</p>
            ...
            <div style={{ background:"rgba(255,255,255,0.03)", border:"0.5px solid rgba(200,146,10,0.2)", borderRadius:14, padding:"14px 16px", marginBottom:20 }}>
              ...
              {[
                ["Offre",      form.offre === "Presence" ? "Présence — Gratuit" : "⭐ Premium — 20 000 Ar"],
                ["Catégorie",  form.cat],
                ["Spot",       form.nom],
...
const InscriptionBanner = ({ onInscription }) => (
  <div style={{
    ...
    <p style={{ fontSize: 11, color: "rgba(244,247,247,0.5)", lineHeight: 1.6 }}>
      Inscrivez-vous gratuitement et bénéficiez d'une fiche complète visible sur toute l'île.
    </p>
...
export default function WayzaPrestigeApp() {
  ...
  useEffect(() => {
    const loadAds = async () => {
      try {
        ...
        // Afficher le popup placeholder "Votre pub ici" si aucune pub active
        if (!normalized.find(a => a.type === "popup")) {
          setTimeout(() => { setActiveAd(null); setShowPopup(true); }, 2800);
        }
      } catch {
        // Afficher le popup placeholder en cas d'erreur
        setTimeout(() => { setActiveAd(null); setShowPopup(true); }, 2800);
      }
    };
    loadAds();
  ...
  const renderMain = () => {
    ...
    if (tab === "home") return <HomeScreen city={city} setCity={setCity} onSpot={setSelectedSpot} onMag={a => { setSelectedArticle(a); setTab("mag"); }} onTab={t => { setTab(t); setSelectedSpot(null); setSelectedArticle(null); }} ads={ads} spots={spots} onInscription={() => setShowInscription(true)} />;
    if (tab === "mag") {
    ...
    <div className="wayza-app-root" style={{
      ...
      <div className="wayza-main-layout" style={{ flex: 1, overflow: "hidden", display: "flex", flexDirection: isDesktop ? "row" : "column", minHeight: 0 }}>
        ...
        <div className="wayza-sidebar" style={{ display: isDesktop ? "flex" : "none", flexDirection: "column", width: 240, minWidth: 240, background: "rgba(0,0,0,0.3)", borderRight: "0.5px solid rgba(200,146,10,0.15)", padding: "24px 16px", gap: 4, overflowY: "auto" }}>
          ...
          {CATEGORIES.filter(c => c.name !== "Wayz'Mag" && c.name !== "Urgences").map(cat => {
      ...
      {/* Bottom nav */}
      <div className="wayza-bottom-nav" style={{ position: "absolute", bottom: 0, left: 0, right: 0, padding: isMobileLandscape ? "6px 20px 8px" : "12px 20px 20px", paddingTop: isMobileLandscape ? "6px" : "24px", background: "linear-gradient(to top, #0D2B30 70%, transparent)", flexShrink: 0, display: isDesktop ? "none" : "block", overflow: "visible" }}>
          ...
          {navItems.map(item => {
            ...
            return (
              <button key={item.id} onClick={() => { setTab(item.id); setSelectedSpot(null); setSelectedArticle(null); }} style={{
                ...
                {item.id === "mag" ? (
                  <div style={{
                    width: 44, height: 44, marginTop: -18,
                    ...
                    <Icon name="sparkles" size={18} color={active ? "#0D2B30" : "#C8920A"} />
                  </div>
                ) : (
