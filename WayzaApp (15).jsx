...
const CATEGORIES = [
  ...
  { id: 4, name: "Wayz'Mag",              icon: "◈",  glow: "#C8920A", sub: "Édition prestige",      img: null },
  ...
  { id: 8, name: "Urgences",              icon: "⊕",  glow: "#8B1A1A", sub: "Contacts essentiels",   img: null },
];
...
// ─── SÉLECTEUR DE VILLE avec géolocalisation ─────────────────
const VILLES_LISTE = [
...
const CitySelector = ({ city, setCity, spots }) => {
  const [open, setOpen] = useState(false);
  const [geoLoading, setGeoLoading] = useState(false);
  const [nearbyCity, setNearbyCity] = useState(null);

  // Correspondances coordonnées → villes
  const GPS_VILLES = [
    { name: "Tana",         lat: -18.91, lng: 47.53, radius: 60 },
    { name: "Nosy Be",      lat: -13.32, lng: 48.27, radius: 30 },
    { name: "Sainte Marie", lat: -17.09, lng: 49.86, radius: 25 },
    { name: "Taolagnaro",   lat: -25.03, lng: 46.98, radius: 40 },
    { name: "Mahajanga",    lat: -15.72, lng: 46.32, radius: 50 },
    { name: "Toamasina",    lat: -18.14, lng: 49.40, radius: 50 },
    { name: "Fianarantsoa", lat: -21.45, lng: 47.09, radius: 50 },
  ];

  const deg2km = (lat1, lng1, lat2, lng2) => {
    const R = 6371;
  ...
  const handleGeo = () => {
    ...
    navigator.geolocation.getCurrentPosition(
      (pos) => {
        ...
        GPS_VILLES.forEach(v => {
          const d = deg2km(lat, lng, v.lat, v.lng);
          if (d < v.radius && d < minDist) { minDist = d; closest = v.name; }
        });
        if (closest) {
          setCity(closest === "Tana" ? "Tana" : closest);
          setNearbyCity(closest);
        } else {
          setNearbyCity("introuvable");
        }
        setGeoLoading(false);
        setOpen(false);
      },
  ...
  const currentVille = VILLES_LISTE.find(v => v.value === city) || VILLES_LISTE[0];
  ...
  return (
    <div style={{ position: "relative" }}>
      ...
      <div style={{ display: "flex", gap: 8, alignItems: "center" }}>
        ...
        >
          <span style={{ fontSize: 16 }}>{currentVille.emoji}</span>
          <div style={{ flex: 1, textAlign: "left" }}>
            <p style={{ fontSize: 8, color: "rgba(200,146,10,0.7)", letterSpacing: "0.2em", fontWeight: 700, marginBottom: 1 }}>CHOISISSEZ VOTRE VILLE</p>
            <p style={{ fontSize: 13, fontWeight: 700, color: "#F4F7F7" }}>{currentVille.label}</p>
          </div>
          <span style={{ fontSize: 10, color: "rgba(200,146,10,0.6)", transform: open ? "rotate(180deg)" : "none", transition: "transform 0.2s" }}>▼</span>
        </button>
        ...
        <button
          ...
          style={{
            ...
            background: nearbyCity && nearbyCity !== "introuvable" ? "rgba(46,204,138,0.15)" : "rgba(255,255,255,0.05)",
            border: nearbyCity && nearbyCity !== "introuvable" ? "0.5px solid rgba(46,204,138,0.4)" : "0.5px solid rgba(255,255,255,0.15)",
      ...
      {nearbyCity === "introuvable" && (
        <p style={{ fontSize: 10, color: "rgba(244,247,247,0.4)", marginTop: 6, paddingLeft: 4, fontStyle: "italic" }}>
          Position hors des villes WAYZA — affichage île entière
        </p>
      )}
      ...
      {open && (
        <div style={{
          ...
          {VILLES_LISTE.map((v, i) => {
            const count = v.value === "Toute l'île" ? spots.length
              : v.value === "Tana" ? spots.filter(s => s.ville === "Antananarivo").length
              : spots.filter(s => s.ville === v.value).length;
            ...
            return (
              <button key={v.value} onClick={() => { setCity(v.value); setOpen(false); }} style={{
                ...
                <span style={{ fontSize: 9, color: "rgba(244,247,247,0.3)", letterSpacing: "0.1em" }}>
                  {count > 0 ? `${count} spot${count > 1 ? "s" : ""}` : "bientôt"}
                </span>
                {active && <span style={{ color: "#C8920A", fontSize: 12 }}>✓</span>}
              </button>
            );
          })}
        </div>
  ...
  return (
    <div style={{ flex: 1, overflowY: "auto", overflowX: "hidden", paddingBottom: 20, minHeight: 0 }}>
      ...
      <div style={{ padding: "0 16px 20px" }}>
        <CitySelector city={city} setCity={setCity} spots={spots} />
      </div>
                ...
                <div style={{ position: "absolute", inset: 0, padding: "8px 8px 9px", display: "flex", flexDirection: "column", justifyContent: "space-between" }}>
                  ...
                  <div>
                    ...
                    <p style={{ fontSize: 8, color: active ? cat.glow : "rgba(255,255,255,0.4)", letterSpacing: "0.05em" }}>
                      {isNavTab ? "→ VOIR" : isEmpty ? "BIENTÔT" : cat.sub}
                    </p>
                  </div>
                </div>
              </div>
